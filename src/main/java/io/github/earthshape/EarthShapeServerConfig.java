...
public final class EarthShapeServerConfig {
    ...
    static {
        ...
        TEMPERATURE_VERTICAL_SCALE = builder
                .comment("Vertical expansion of earth_temperature.png around the equator. 1.12 keeps southern Africa in its intended warm band when using the expanded 6000x3400 world map.")
                .defineInRange("temperatureVerticalScale", 1.12D, 0.75D, 1.50D);
        ...
        COAST_HEIGHT_FADE_BLOCKS = builder.comment("Distance from an ocean coast over which heightmap relief rises from a negative shoreline grade.").defineInRange("coastHeightFadeBlocks", 320, 20, 1024);
        RIVER_HEIGHT_FADE_BLOCKS = builder.comment("Distance from a source river bank over which heightmap relief rises from the water level.").defineInRange("riverHeightFadeBlocks", 160, 20, 1024);
        ...
        RIVER_MAXIMUM_DEPTH_BLOCKS = builder.comment("Maximum source-river bed drop in blocks before vanilla aquifer and surface generation.").defineInRange("riverMaximumDepthBlocks", 6, 2, 32);
        builder.pop();
        ...
        RIVER_GAP_BRIDGE_PIXELS = builder.comment("Maximum missing source pixels joined only between similarly directed river strokes.  This repairs broken thin lines without joining nearby rivers.").defineInRange("gapBridgePixels", 8, 0, 16);
        RIVER_CHANNEL_CONTINENTALNESS = builder.comment("Continentalness at the center of a source-layer river. Lower values make a reliable shallow water channel before normal terrain generation.").defineInRange("channelContinentalness", -0.42D, -0.80D, -0.05D);
        RIVER_BANK_FADE_BLOCKS = builder.comment("Physical block distance used to grade each river bank into surrounding terrain. This must not scale with map pixels, or rivers become oversized at large blocksPerPixel values.").defineInRange("bankFadeBlocks", 48, 2, 128);
        RIVER_MINIMUM_INLAND_BLOCKS = builder.comment("Required land margin on all four sides of a source river. Prevents shoreline strokes from turning small islands into river-only biomes.").defineInRange("minimumInlandBlocks", 24, 4, 128);
        RIVER_CHANNEL_EDGE_FADE_BLOCKS = builder.comment("Physical fade from a source river bed into its bank. A gradual value prevents vertical water-side cliffs.").defineInRange("channelEdgeFadeBlocks", 32, 0, 128);
        builder.pop();
        builder.push("desert_water");
        DESERT_WATER_REDUCTION_ENABLED = builder.comment("Suppress generated surface lake features and minor source rivers in terrain.bmp desert areas.").define("enabled", true);
        DESERT_RIVER_WIDTH_SCALE = builder.comment("Width multiplier for source rivers crossing desert. Values above 0.30 are safely capped to prevent desert river networks becoming lakes.").defineInRange("riverWidthScale", 0.20D, 0.0D, 1.0D);
        DESERT_MINIMUM_RIVER_WIDTH_BLOCKS = builder.comment("Minimum pre-cap source river width allowed in desert. Smaller lines become ordinary desert terrain.").defineInRange("minimumRiverWidthBlocks", 20, 1, 128);
        DESERT_MAXIMUM_RIVER_WIDTH_BLOCKS = builder.comment("Maximum final water width for a surviving desert river. Prevents broad source strokes and loops from becoming desert lakes.").defineInRange("maximumRiverWidthBlocks", 18, 4, 64);
        builder.pop();
        SPEC = builder.build();
    }
